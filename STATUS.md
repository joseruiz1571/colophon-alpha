...
| C46 | partial | `.github/workflows/ci.yml` defines exactly the ten required steps (install, typecheck, bun test, opa test, opa fmt --diff, schema validation, bun run demo, colophon bundle verify, keyless Cosign sign and verify, secret scan). All ten were exercised individually in this build environment except the keyless-Cosign step (see C42); whether the workflow is green on `main` as a whole can only be confirmed by an actual run on GitHub after this work is pushed. |
...
| A2 | done | `gitleaks detect --no-git -v --source .` against the committed tree → `no leaks found` (run locally after removing local, gitignored test scratch directories that briefly held throwaway, password-encrypted Cosign test keys — never committed; `test/tmp-*/` is now gitignored so this can't recur). CI's own "secret scan" step runs the same tool. |
